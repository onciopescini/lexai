...
export default function SocialCard({ content, sourceTitle, onClose }: SocialCardProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-white rounded-[32px] shadow-2xl max-w-4xl w-full flex flex-col md:flex-row overflow-hidden relative">
        <button 
          onClick={onClose}
        ...
        <div className="flex-1 p-8 bg-slate-50 border-r border-slate-100 flex flex-col">
          ...
          <div className="flex gap-2 mb-6 p-1 bg-slate-200/50 rounded-[24px] w-max">
            <button 
              onClick={() => setFormat('story')}
              className={`px-4 py-2 rounded-[20px] text-sm font-bold transition-all ${format === 'story' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Story (9:16)
            </button>
            <button 
              onClick={() => setFormat('post')}
              className={`px-4 py-2 rounded-[20px] text-sm font-bold transition-all ${format === 'post' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Post (1:1)
            </button>
          </div>
          ...
          <div className="space-y-4 flex-1">
             {!downloadUrl ? (
                <button 
                  ...
                  className="w-full py-4 rounded-[24px] bg-blue-600 hover:bg-blue-700 text-white font-bold shadow-lg shadow-blue-500/30 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {isGenerating ? (
                    <>
                ...
                <div className="flex flex-col gap-3">
                  <a 
                    ...
                    className="w-full py-4 rounded-[24px] bg-green-500 hover:bg-green-600 text-white font-bold shadow-lg shadow-green-500/30 transition-all flex items-center justify-center gap-2 text-center"
                  >
                  ...
                  <button 
                    ...
                    className="w-full py-3 rounded-[24px] bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold transition-all"
                  >
                    Modifica formato
                  </button>
                </div>
          ...
          >
             ...
             >
                ...
                <div className="flex items-center gap-3 mb-8 z-10 shrink-0">
                  <div className="w-10 h-10 rounded-[24px] bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-lg">
                    <Image src="/atena-logo-new.jpeg" alt="Atena" width={30} height={10} className="mix-blend-screen object-contain" />
                  </div>
                  <div>
                ...
                <div className="mt-8 pt-6 border-t border-slate-100 flex items-end justify-between z-10 shrink-0 opacity-80">
                  ...
                  <div className="flex flex-col items-center gap-1">
                     ...
                     <div className="w-12 h-12 bg-white rounded-[20px] border border-slate-200 p-1 flex items-center justify-center">
