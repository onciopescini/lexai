...
function FeatureCard({ icon, title, description, tag }: { icon: string; title: string; description: string; tag?: string }) {
  return (
    <div className="group relative p-6 rounded-2xl bg-gradient-to-b from-[#111113] to-[#0a0a0b] border border-white/5 hover:border-blue-500/30 transition-all duration-500 overflow-hidden">
      <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/5 rounded-full filter blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
      <div className="relative z-10">
        <div className="flex items-center gap-3 mb-3">
          <span className="text-2xl">{icon}</span>
          {tag && (
            <span className="text-[9px] uppercase font-bold tracking-widest text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded-sm">{tag}</span>
          )}
        </div>
...
function ProcessStep({ step, title, description }: { step: number; title: string; description: string }) {
  return (
    <div className="flex flex-col items-center text-center group">
      <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center text-xl font-bold text-white shadow-[0_0_20px_rgba(37,99,235,0.4)] group-hover:shadow-[0_0_30px_rgba(37,99,235,0.6)] transition-all mb-4">
        {step}
      </div>
...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#050505] text-white font-sans selection:bg-blue-500/30 overflow-x-hidden">
      
      {/* Navbar Minimal - Fixed at Top */}
      <nav className="w-full flex items-center justify-between p-6 max-w-6xl mx-auto border-b border-white/5 shrink-0 z-50 bg-[#050505]/80 backdrop-blur-md sticky top-0">
        ...
        <div className="flex items-center gap-6 text-sm text-white/50">
          ...
          <a href="#search" className="hover:text-white transition-colors">Ricerca</a>
            ...
            <>
              ...
              <section className="text-center mt-20 mb-6 animate-fade-in-up">
                ...
                <p className="text-sm text-white/25 max-w-lg mx-auto">
                  Costituzione · Codice Civile · Normattiva · Eurlex · Aggiornamenti Live
                </p>
              </section>

              {/* SEARCH AREA */}
              <section id="search" className="w-full max-w-3xl mx-auto mb-16">
                {/* Source Filter Tabs */}
                ...
                <div className="w-full relative group">
                  ...
                  <form onSubmit={handleSearch} className="relative flex items-center bg-[#141415] border border-white/10 rounded-2xl shadow-2xl p-2 focus-within:border-blue-500/50 transition-all">
                    ...
                    <button 
                      ...
                      className="absolute right-2 p-2.5 bg-white text-black rounded-xl font-medium hover:bg-gray-200 transition-colors disabled:opacity-30 flex items-center justify-center"
                    >
                      {loading ? (
              ...
              <section className="w-full max-w-4xl mx-auto mb-20">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 p-6 rounded-2xl bg-[#0a0a0b]/80 border border-white/5 backdrop-blur-md">
                  <AnimatedStat value="3.600" label="Fonti Indicizzate" suffix="+" />
                  <AnimatedStat value="24/7" label="Monitoraggio Live" />
                  <AnimatedStat value="5" label="Database Legali" />
                  <AnimatedStat value="< 2s" label="Tempo Risposta" />
                </div>
              </section>

              {/* 3-STEP PROCESS (BabyLoveGrowth inspired) */}
              <section className="w-full max-w-4xl mx-auto mb-20">
              ...
              {/* FEATURE SHOWCASE */}
              <section id="features" className="w-full max-w-5xl mx-auto mb-20">
                <div className="text-center mb-12">
                  <h2 className="text-2xl md:text-3xl font-bold text-white/90 mb-3">
                    Tecnologia di Classe Enterprise
                  </h2>
                  <p className="text-sm text-white/40 max-w-lg mx-auto">Ogni funzionalità è progettata per il professionista legale che pretende il massimo.</p>
                </div>
              ...
              <section className="w-full max-w-4xl mx-auto mb-20">
                <div className="text-center mb-8">
                  <p className="text-xs text-white/25 uppercase tracking-widest font-semibold">Powered by</p>
                </div>
                <div className="flex flex-wrap justify-center gap-8 items-center opacity-40">
                  <div className="flex items-center gap-2 text-sm text-white/60">
                    <span className="text-lg">🔮</span> Google Gemini 2.5
                  </div>
                  <div className="flex items-center gap-2 text-sm text-white/60">
                    <span className="text-lg">🔍</span> Perplexity Sonar
                  </div>
                  <div className="flex items-center gap-2 text-sm text-white/60">
                    <span className="text-lg">⚡</span> Supabase + pgvector
                  </div>
                  <div className="flex items-center gap-2 text-sm text-white/60">
                    <span className="text-lg">🎨</span> Google Imagen 4
                  </div>
                  <div className="flex items-center gap-2 text-sm text-white/60">
                    <span className="text-lg">▲</span> Vercel Edge
                  </div>
                </div>
              </section>

              {/* CTA FINALE */}
              <section className="w-full max-w-3xl mx-auto mb-32 text-center">
                <div className="p-10 rounded-3xl bg-gradient-to-b from-blue-900/20 to-[#0a0a0b] border border-blue-500/20 relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    ...
                    <a href="#search" className="inline-flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold rounded-xl hover:from-blue-500 hover:to-indigo-500 transition-all shadow-lg shadow-blue-900/30 active:scale-95">
                      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
